...
try:
    ...
    db_uri = f'postgresql://{user}:{password}@localhost:5432/airbnb_db'
...
def statistics(city_id, nbh_id):
    ...
    if nbh_id != "0":
        results =  db.session.query(Nbh_Overview.nbh_id.label("nbh_id"), \
                            ...
                            func.avg(Listings_Info.night_price).label("night_price"),\
                            func.avg(Listings_Info.cleaning_fee).label("cleaning_fee"),\
                            func.avg(Listings_Info.nights_booked).label("nights_booked"),\
                            func.avg(Listings_Info.rental_income).label("rental_income"),
                            func.avg(Listings_Info.total_reviews).label("total_reviews"),\
                            func.count(Listings_Info.night_price).label("listing_count"))\
                    .join(Nbh_Insights, Nbh_Overview.nbh_id==Nbh_Insights.nbh_id)  \
                    .join(Listings_Info, Nbh_Overview.nbh_id==Listings_Info.nbh_id)  \
...
def rentalType(city_id, nbh_id):
    ...
    if nbh_id != "0":
        results =  db.session.query(
                        Listings_Info.property_type.label("property_type"),\
                        func.avg(Listings_Info.night_price).label("night_price"),\
                        func.avg(Listings_Info.cleaning_fee).label("cleaning_fee"),\
                        func.avg(Listings_Info.nights_booked).label("nights_booked"),\
                        func.avg(Listings_Info.rental_income).label("rental_income"))\
                        .filter(Listings_Info.nbh_id == nbh_id) \
                        .group_by(Listings_Info.property_type)
    else:
        results =  db.session.query(
                        Listings_Info.property_type.label("property_type"),\
                        func.avg(Listings_Info.night_price).label("night_price"),\
                        ...
                        func.avg(Listings_Info.nights_booked).label("nights_booked"),\
                        func.avg(Listings_Info.rental_income).label("rental_income"))\
                    .join(City_Nbh, City_Nbh.nbh_id ==  Listings_Info.nbh_id)\
                    .filter(City_Nbh.city_id == city_id)\
                    .group_by(Listings_Info.property_type)
